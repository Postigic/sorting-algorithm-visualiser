...
const ALGOS = [
    ...
    },
    {
        ...
        inplace: true,
        metrics: { swaps: false },
        desc: "Insertion sort generalised to compare elements at a shrinking gap distance, performing a final gap-1 pass to finish. The large initial gaps move elements close to their final positions quickly, making the last insertion sort pass cheap. This implementation uses simple halving for the gap sequence, giving O(n²) worst case; better sequences like Ciura's improve this significantly in practice. The commonly-quoted O(n^1.5) average case belongs to different increment sequences (Papernov-Stasevitch, Hibbard, Knuth); for the plain halving sequence used here, the average case has never been resolved and remains an open problem in algorithm analysis. A good middle ground that is simple to implement and faster than pure insertion sort on larger inputs.",
    },
    {
        name: "Cycle Sort",
        fn: cycleSort,
    ...
    },
    {
        name: "Stooge Sort",
        fn: stoogeSort,
        time_worst: "Θ(n^(log 3 / log 1.5))",
        time_avg: "Θ(n^(log 3 / log 1.5))",
        time_best: "Θ(n^(log 3 / log 1.5))",
        aux: "O(log n)",
        stable: false,
        inplace: true,
        metrics: { writes: false },
        desc: "Recursively sorts the first two-thirds of the array, then the last two-thirds, then the first two-thirds again. Named after the Three Stooges. The three overlapping recursive calls are provably inefficient at approximately O(n^2.71), making it slower than bubble sort and every other quadratic algorithm. Its recursive structure is entirely data-independent, so worst, average, and best case all coincide exactly. Exists as a theoretical example of a correctly sorting algorithm that is nonetheless worse than naive approaches.",
    },
    {
        name: "Slowsort",
        fn: slowsort,
        ...
        time_best: "Ω(n^(log n / (2+ε)))",
        aux: "O(log n)",
        stable: false,
        inplace: true,
        metrics: { writes: false },
        desc: "Based on the multiply-and-surrender paradigm, deliberately the opposite of divide and conquer. Recursively sorts both halves, promotes the maximum to the end, then recursively sorts everything except the last element, including re-sorting already partially sorted regions. The redundant overlap makes it superpolynomial. Like Stooge sort, the recursion pattern never branches on the data itself, so all three cases are identical; unlike Stooge sort, its exact growth rate is an open problem. Broder and Stolfi proved only that it sits between Ω(n^(log n / (2+ε))) and O(n^(log n / 2)) for any ε > 0, so a tight Θ bound has never been established. Designed specifically as a joke to be provably non-optimal while still being a correct sorting algorithm.",
    },
    {
        name: "Comb Sort",
        fn: combSort,
        ...
        inplace: true,
        metrics: { writes: false },
        desc: "Bubble sort generalised to compare elements at a gap distance, shrinking the gap by a factor of 1.3 each pass until it reaches 1 and finishes like a regular bubble sort. The large initial gaps eliminate turtles; small elements near the end that make bubble sort slow, early in the process. The n²/2^p figure (p being the number of increments) is a proven lower bound on the average case, not an upper bound, and no matching upper bound has been established. A practical improvement over bubble sort for the same reason shell sort improves on insertion sort.",
    },
    {
        name: "Comb Sort 11",
        fn: combSort11,
        ...
        inplace: true,
        metrics: { writes: false },
        desc: "Comb sort with an empirical adjustment that skips gap values of 9 and 10, jumping straight to 11. These gap sizes are known to leave turtles unresolved and perform poorly in practice. The skip improves average performance measurably, though like standard comb sort the n²/2^p figure is only a proven lower bound rather than a tight one. The name refers to 11 being the smallest gap size this variant will actually use.",
    },
    {
    ...
    },
    {
        ...
        inplace: false,
        metrics: { writes: false },
        desc: "A recursive descent into madness built on bogosort. To sort k elements, it randomly shuffles them, recursively bogobogosorts the first k-1, then checks if all k are now in order, restarting the whole process if not. Each level depends on the level below completing first, multiplying an already incomprehensible runtime at every step. Working the recurrence out (as in the original esolang writeup) gives O(n · (n!)ⁿ) for the average case, though the exact growth rate has been debated since the derivation makes simplifying assumptions at each recursive level. Even small arrays are effectively impossible to sort in any reasonable timeframe. Not a joke about bad sorting; a joke about what happens when you recurse a joke.",
    },
    {
        name: "Bozo Sort",
        fn: bozoSort,
    ...
    },
    {
        ...
        desc: "Uses statistical inference to avoid unnecessary computation. Samples twenty random adjacent pairs and classifies them as either evidence of order or statistical outliers. Continues sampling until the evidence supports the hypothesis that the array is already sorted. Best case is a fixed twenty-pair sample regardless of n, so it's constant rather than merely bounded above. Inversions are treated as anomalies rather than problems, allowing the algorithm to replace sorting with confidence.",
    },
    {
        name: "Council Sort",
        fn: councilSort,
